...
def serve(stdin=None, stdout=None) -> int:
    ...
    Overrideable paths via env or CLI:
      SKILLHUB_REGISTRY       path to skills.jsonl
      SKILLHUB_COLLECTIONS    path to collections.json
    """
...
def _apply_path_overrides() -> None:
    """Allow env SKILLHUB_REGISTRY / SKILLHUB_COLLECTIONS to override defaults.

    Mutates module-level globals in already-imported modules so the
    override takes effect even though the modules were imported at server
    startup (which used default paths).
    """
    reg = os.environ.get("SKILLHUB_REGISTRY")
    coll = os.environ.get("SKILLHUB_COLLECTIONS")
    if reg or coll:
        import skillhub.cli as _cli_mod
        import skillhub.collections as _coll_mod
        if reg:
            p = Path(reg).resolve()
            _cli_mod.REGISTRY_PATH = p  # type: ignore[attr-defined]
        if coll:
            p = Path(coll).resolve()
            _coll_mod.COLLECTIONS_PATH = p  # type: ignore[attr-defined]


if __name__ == "__main__":
    # Optional CLI: --registry <path> --collections <path>
    import argparse
    parser = argparse.ArgumentParser(prog="skillhub-mcp")
    parser.add_argument("--registry", help="path to skills.jsonl")
    parser.add_argument("--collections", help="path to collections.json")
    args = parser.parse_args()
    if args.registry:
        os.environ["SKILLHUB_REGISTRY"] = args.registry
    if args.collections:
        os.environ["SKILLHUB_COLLECTIONS"] = args.collections
    sys.exit(serve())
